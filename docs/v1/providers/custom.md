...
When authenticating with a secure JWT, the data encoded in the token will be accessible in the [Security Rules](https://www.firebase.com/docs/ios/guide/securing-data.html) as the `auth` variable. This allows rules to be written which grant access based on the data within the token.
